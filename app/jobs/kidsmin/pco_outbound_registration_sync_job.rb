  ...
  class PcoOutboundRegistrationSyncJob < ApplicationJob
    def perform(registration_id)
      ...
      unless child.pco_person_id.present? && event.pco_event_id.present?
        Rails.logger.info(
          "[Kidsmin] PcoOutboundRegistrationSyncJob skipped for registration #{registration_id} " \
      ...
      if event.pco_source == "check_ins"
        ...
        Rails.logger.info(
          "[Kidsmin] PcoOutboundRegistrationSyncJob: calendar event #{event.pco_event_id} — " \
      ...
      Rails.logger.info("[Kidsmin] PcoOutboundRegistrationSyncJob complete for registration #{registration_id}")
    rescue ActiveRecord::RecordNotFound
      Rails.logger.warn("[Kidsmin] PcoOutboundRegistrationSyncJob: registration #{registration_id} not found, discarding")
    rescue PcoError => e
      Rails.logger.error("[Kidsmin] PcoOutboundRegistrationSyncJob failed for registration #{registration_id}: #{e.message}")
      raise
    end
    ...
    def sync_as_checkin(client, registration, child, event)
      ...
      unless event_time
        Rails.logger.warn("[Kidsmin] No EventTime found for PCO event #{event.pco_event_id} — skipping check-in creation")
        return
      end
