namespace :kidsmin do
  ...
  task first_deploy: :environment do
    ...
    abort "Set ADMIN_EMAIL env var. Usage: ADMIN_EMAIL=you@example.com rails kidsmin:first_deploy" if email.blank?

    puts "\n=== kidsmin first-deploy setup ===\n\n"
    ...
    puts "  Families:    #{Kidsmin::Family.count}"
    puts "\nRun `rails kidsmin:setup_check` to verify env vars.\n\n"
  end


  desc "Promote a user to admin by email. Usage: rails kidsmin:make_admin[email@example.com]"
  task :make_admin, [:email] => :environment do |_, args|
    email = args[:email]&.strip
    abort "Usage: bundle exec rails kidsmin:make_admin[email@example.com]" if email.blank?
  ...
  task :setup_check do
    ...
    puts "  4. In Railway shell: ADMIN_EMAIL=you@example.com rails kidsmin:first_deploy"
