module Kidsmin
  class PcoInboundEventsSyncJob < ApplicationJob
    def perform
      ...
      Rails.logger.info("[Kidsmin] PcoInboundEventsSyncJob complete")
    rescue PcoError => e
      Rails.logger.error("[Kidsmin] PcoInboundEventsSyncJob failed: #{e.message}")
      raise
    end
    ...
    def sync_calendar_events(client)
      ...
      Rails.logger.warn("[Kidsmin] Calendar events sync skipped (#{e.status}): #{e.message}")
    end
    ...
    def sync_checkins_events(client)
      ...
      Rails.logger.warn("[Kidsmin] CheckIns events sync skipped (#{e.status}): #{e.message}")
    end
    ...
    def upsert_event(pco_event, source)
      ...
      Rails.logger.error("[Kidsmin] Failed to upsert event pco_id=#{pco_event["id"]}: #{e.message}")
    end
