...
RSpec.describe Kidsmin::Family, type: :model do
  describe "validations" do
    it "accepts a valid email" do
      expect(build(:kidsmin_family, email: "good@example.com")).to be_valid
    end

    it "rejects an invalid email" do
      family = build(:kidsmin_family, email: "not-an-email")
      expect(family).not_to be_valid
      expect(family.errors[:email]).to be_present
    end

    it "allows a blank email" do
      expect(build(:kidsmin_family, email: "")).to be_valid
    end
  end

  describe "#primary_contact_name" do
    it "concatenates first and last name" do
      family = build(:kidsmin_family,
