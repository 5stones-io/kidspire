...
module Kidsmin
  class PcoClient
    ...
    def refresh_token!
      log "[Kidsmin::PcoClient] Refreshing access token"
      ...
      log "[Kidsmin::PcoClient] Token refreshed, expires #{@integration.expires_at}"
    end
    ...
    def log_request(method, url, options)
      ...
      Rails.logger.debug("[Kidsmin::PcoClient] → #{method.upcase} #{url} #{safe.inspect}")
    end

    def log_response(response)
      return unless debug?
      preview = response.body&.slice(0, 500)
      Rails.logger.debug("[Kidsmin::PcoClient] ← #{response.code} #{preview}")
    end
